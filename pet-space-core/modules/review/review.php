...
function psc_gps_verify_store( float $user_lat, float $user_lng, int $store_id ): array {
    ...
    $distance_label   = $distance_rounded >= 1000
        ? number_format( $distance_rounded / 1000, 1 ) . 'km'
        : $distance_rounded . 'm';
    ...
    return [
        ...
        'message'  => $distance <= $radius
            ...
            : "매장에서 {$distance_label} 떨어져 있습니다 (허용 반경: {$radius}m)",
    ];
}
...
function psc_render_reviews_page( int $store_id ): void {
            ...
            <div class="psc-review-list" style="display:flex;flex-direction:column;gap:16px;">
                ...
                <div class="psc-rv-card" style="background:#fff;border:1px solid #e5e8eb;border-radius:16px;padding:20px;">
                    ...
                    <?php if ( ! empty( $rv_images ) ) : ?>
                        <?php
                        $img_urls = array_map( fn($img) => esc_url( $img->url ), $rv_images );
                        $img_json = esc_attr( wp_json_encode( $img_urls ) );
                        ?>
                        <div class="psc-rv-images" data-gallery="<?php echo $img_json; ?>">
                            <?php foreach ( $rv_images as $idx => $img ) : ?>
    ...
    <?php
    if ( function_exists( 'psc_review_script' ) ) echo psc_review_script();
    get_footer();
}
