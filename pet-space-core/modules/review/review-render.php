...
function psc_shortcode_review_write( array $atts ): string {
    ...
    $default_id = ( $current_id && get_post_type( $current_id ) === 'store' ) ? $current_id : 0;
    $atts       = shortcode_atts( [ 'id' => $default_id ], $atts, 'store_review_write' );
    $post_id    = isset( $_GET['store_id'] ) ? (int) $_GET['store_id'] : (int) $atts['id'];
    if ( ! $post_id ) return '';
